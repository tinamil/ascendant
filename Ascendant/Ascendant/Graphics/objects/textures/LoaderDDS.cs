...
using ExtTextureCompressionS3tc = OpenTK.Graphics.OpenGL.ExtTextureCompressionS3tc;
...
namespace Examples.TextureLoaders {
    ...
    static class ImageDDS {
        ...
        public static void LoadFromDisk(string filename, out uint texturehandle, out TextureTarget dimension) {
            ...
            _BytesPerBlock = 0;
            _PixelInternalFormat = PixelInternalFormat.Srgb8;
            ...
            try // Exceptions will be thrown if any Problem occurs while working on the file. 
            {
                ...
                for (int Slices = 0; Slices < _Depth; Slices++) {
                    ...
                    for (int Level = 0; Level < _MipMapCount; Level++) // start at base image
                    {
                        ...
                        if (Width > 2 && Height > 2) { // Note: there could be a potential problem with non-power-of-two cube maps
                            ...
                            if (!TextureLoaderParameters.FlipImages) { // no changes to the image, copy as is
                                Array.Copy(_RawDataFromFile, Cursor, RawDataOfSurface, 0, SurfaceSizeInBytes);
                            } else {  // Turn the blocks upside down and the rows aswell, done in a single pass through all blocks
                            ...
                            GLError = GL.GetError();
                            if (TextureLoaderParameters.Verbose)
                                Trace.WriteLine("GL: " + GLError.ToString() + " Level: " + Level + " DXTn: " + ((compressed == 1) ? "Yes" : "No") + " Frmt:" + (OpenTK.Graphics.OpenGL.ExtTextureCompressionS3tc)internalformat + " " + width + "*" + height);
                    ...
                    int baseLevel = 0;
                    GL.TexParameterI(dimension, (TextureParameterName)All.TextureBaseLevel, ref baseLevel);
                    GL.TexParameterI(dimension, (TextureParameterName)All.TextureMaxLevel, ref trueMipMapCount);

                    int TexMaxLevel;
                    GL.GetTexParameterI(dimension, GetTextureParameter.TextureMaxLevel, out TexMaxLevel);
                ...
                int mini = (int)TextureLoaderParameters.MinificationFilter;
                int magni = (int)TextureLoaderParameters.MagnificationFilter;
                GL.TexParameterI(dimension, TextureParameterName.TextureMinFilter, ref mini);
                GL.TexParameterI(dimension, TextureParameterName.TextureMagFilter, ref magni);
                
                int sWrap =  (int)TextureLoaderParameters.WrapModeS;
                int tWrap =  (int)TextureLoaderParameters.WrapModeT;
                GL.TexParameterI(dimension, TextureParameterName.TextureWrapS, ref sWrap);
                GL.TexParameterI(dimension, TextureParameterName.TextureWrapT, ref tWrap);
